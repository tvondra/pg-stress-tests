...
EOF
		...
		for opt in devel_standard_join_search devel_apply_scanjoin_target_to_paths devel_set_rel_pathlist devel_create_ordered_paths devel_create_ordered_paths_parallel devel_add_paths_to_grouping_rel devel_add_paths_to_grouping_rel_parallel devel_create_partial_grouping_paths devel_create_partial_grouping_paths_2 devel_gather_grouping_paths; do
...
set max_parallel_workers_per_gather = 0;
set $opt = on;
SELECT store_plan('$query', '$idx', '$opt', '$t', false, false);
EOF
...
set parallel_setup_cost = 0;
set $opt = on;
SELECT store_plan('$query', '$idx', '$opt', '$t', false, true);
EOF
...
set force_incremental_sort = on;
set $opt = on;
SELECT store_plan('$query', '$idx', '$opt', '$t', true, false);
EOF
...
set force_incremental_sort = on;
set $opt = on;
SELECT store_plan('$query', '$idx', '$opt', '$t', true, true);
EOF
