...
EOF
						...
						echo "btree" $MACHINE $time $rows $dataset $matches $distinct $build $prefetch $ts $is $scan_type uncached $r $value $nfound $cost $duration $best_plan $correct_plan >> $RESULTS 2>&1
...
EOF
						...
						echo "btree" $MACHINE $time $rows $dataset $matches $distinct $build $prefetch $ts $is $scan_type cached-os $r $value $nfound $cost $duration $best_plan $correct_plan >> $RESULTS 2>&1
...
EOF
						...
						cat explain.log >> explain-$scan_type.log

						echo "btree" $MACHINE $time $rows $dataset $matches $distinct $build $prefetch $ts $is $scan_type cached $r $value $nfound $cost $duration $best_plan $correct_plan >> $RESULTS 2>&1
