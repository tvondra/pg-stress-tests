...
echo "test machine time rows dataset matches distinct build prefetch table_size index_size scan_type caching run value nfound cost duration best_plan correct_plan" > $RESULTS
			...
			for prefetch in 0 32 8; do
				...
				for r in `seq 1 $NRUNS`; do
					...
					for scan in indexonlyscan indexonlyscan-clean indexscan bitmapscan seqscan; do
						...
						echo "================================================================================================================" >> $LOGFILE 2>&1
						echo "======================== $time $rows $dataset matches $matches distinct $distinct build $build prefetch $prefetch run $r scan $scan_type ========================" >> $LOGFILE 2>&1
						echo "================================================================================================================" >> $LOGFILE 2>&1
						...
						if [ "$scan" == "indexonlyscan" ]; then
							...
							enable_indexscan='on'
							# override the query, to allow index-only scans (select just "a")
...
EOF
						...
						echo "======================== $time $rows $dataset matches $matches distinct $distinct build $build prefetch $prefetch run $r scan $scan_type / uncached best $best_plan correct $correct_plan ========================" >> $LOGFILE 2>&1
...
EOF
						...
						echo "btree-sort" $MACHINE $time $rows $dataset $matches $distinct $build $prefetch $ts $is $scan_type uncached $r $value $nfound $cost $duration $best_plan $correct_plan >> $RESULTS 2>&1
						...
						echo "======================== $time $rows $dataset matches $matches distinct $distinct build $build prefetch $prefetch run $r scan $scan_type / cached-os best $best_plan correct $correct_plan ========================" >> $LOGFILE 2>&1
...
EOF
						...
						echo "btree-sort" $MACHINE $time $rows $dataset $matches $distinct $build $prefetch $ts $is $scan_type cached-os $r $value $nfound $cost $duration $best_plan $correct_plan >> $RESULTS 2>&1
						...
						echo "======================== $time $rows $dataset matches $matches distinct $distinct build $build prefetch $prefetch run $r scan $scan_type / cached best $best_plan correct $correct_plan ========================" >> $LOGFILE 2>&1
...
EOF
						...
						echo "btree-sort" $MACHINE $time $rows $dataset $matches $distinct $build $prefetch $ts $is $scan_type cached $r $value $nfound $cost $duration $best_plan $correct_plan >> $RESULTS 2>&1
