...
psql $DBNAME -c "create index on t using brin (a oid_bloom_ops) with (pages_per_range = $rangesize)";
...
EOF
    ...
    psql $DBNAME -c "insert into t select ($maxvalue * random())::int from generate_series(1,$ninsert) s(i)";

    # now do random update
    psql $DBNAME -c "update t set a = ($maxvalue * random())::int where random() < $updatefrac";
