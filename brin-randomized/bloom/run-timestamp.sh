...
psql $DBNAME -c "create index on t using brin (a timestamp_bloom_ops) with (pages_per_range = $rangesize)";
...
EOF
    ...
    psql $DBNAME -c "insert into t select random_timestamp(p_years := $nyears, p_months := $nmonths, p_days := $ndays, p_hours := $nhours, p_minutes := $nminutes, p_seconds := $nseconds) from generate_series(1,$ninsert) s(i)";

    # now do random update
    psql $DBNAME -c "update t set a = random_timestamp(p_years := $nyears, p_months := $nmonths, p_days := $ndays, p_hours := $nhours, p_minutes := $nminutes, p_seconds := $nseconds) where random() < $updatefrac";
