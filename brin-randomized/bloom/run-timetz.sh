...
psql $DBNAME -c "create index on t using brin (a timetz_bloom_ops) with (pages_per_range = $rangesize)";
...
EOF
    ...
    psql $DBNAME -c "insert into t select random_timetz(p_hours := $nhours, p_minutes := $nminutes, p_seconds := $nseconds) from generate_series(1,$ninsert) s(i)";

    # now do random update
    psql $DBNAME -c "update t set a = random_timetz(p_hours := $nhours, p_minutes := $nminutes, p_seconds := $nseconds) where random() < $updatefrac";
